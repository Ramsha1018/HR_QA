    ...
    def get_pdf_splits(self, pdf_file: str, filename: str):
        try:
            ...
            textsplit = RecursiveCharacterTextSplitter(
                ...
                length_function=len)
            doc_list = []
            for pg_num, pg in enumerate(pages, start=1):
                pg_splits = textsplit.split_text(pg.page_content)
                for page_sub_split in pg_splits:
                    metadata = {"source": filename, "page_number": pg_num}
...
def conversation_retrieval_chain(query, vectordb):
    ...
    page_number = context['context'][0].metadata['page_number']
    
    # Display the page number in the answer
    response = f"Answer: {response} (Page {page_number})"
